...
public class NameAsCreated
{
	...
	private static final SimpleDateFormat[] formats = {new SimpleDateFormat("yyyy-MM-dd HH.mm.ss", Locale.ENGLISH), new SimpleDateFormat("yyyy-MM-dd HH-mm-ss", Locale.ENGLISH), new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH), new SimpleDateFormat("'Screen Shot' yyyy-MM-dd 'at' HH.mm.ss", Locale.ENGLISH), new SimpleDateFormat("'Photo' MMM dd, hh mm ss aaa", Locale.ENGLISH), new SimpleDateFormat("'Photo' dd-MM-yyyy, hh mm ss", Locale.ENGLISH), new SimpleDateFormat("'Photo' dd-MM-yyyy hh mm ss", Locale.ENGLISH), new SimpleDateFormat("'Video' MMM dd, hh mm ss aaa", Locale.ENGLISH), new SimpleDateFormat("'Video' dd-MM-yyy, hh mm ss", Locale.ENGLISH), new SimpleDateFormat("'Video' dd-MM-yyy hh mm ss", Locale.ENGLISH), new SimpleDateFormat("dd MMM yyy, hh:mm:ss", Locale.ENGLISH)};
	...
	public static NewFile buildName(final File f) throws IOException
	{
		...
		final HashMap<Class<? extends Directory>, DateExtractor<? extends Directory>> tags = new HashMap<>();
				...
				try
				{
					...
					try
					{
						...
						for(final var xmpDirectory : metadata.getDirectoriesOfType(XmpDirectory.class))
						{
							if(!maybeZoneId.isPresent())
							{
								final Map<String, String> xmpValues = xmpDirectory.getXmpProperties();
								if(xmpValues.containsKey("exif:GPSLatitude") && xmpValues.containsKey("exif:GPSLongitude"))
								{
									var lat = getAngle(xmpValues.get("exif:GPSLatitude"));
									var lon = getAngle(xmpValues.get("exif:GPSLongitude"));
	...
	private static Angle getAngle(String s)
	{
		Pattern pattern = Pattern.compile("(\\d{1,3}),(\\d{1,2})\\.(\\d+)([NESW])");
	...
	private static double getMultiplicator(String group)
	{
		switch(group)
		{
			case "W":
